...
const SpeechInput = forwardRef<HTMLDivElement, SpeechInputProps>(
  function SpeechInput(
    ...
    const scribe = useScribe({
      modelId, baseUri, commitStrategy, vadSilenceThresholdSecs, vadThreshold,
      minSpeechDurationMs, minSilenceDurationMs, languageCode, audioFormat, sampleRate,
      microphone,
    ...
    const start = useCallback(async () => {
      ...
      transcriptsRef.current = { partialTranscript: "", committedTranscripts: [] }
    ...
    return (
      <SpeechInputContext.Provider value={contextValue}>
        <div
          ref={ref}
          className={cn(
            "relative inline-flex items-center overflow-hidden rounded-full border transition-all duration-200",
            scribe.isConnected
  ...
  function SpeechInputRecordButton(
    ...
    return (
      <Button
        ...
        variant={variant}
        onClick={(e) => {
          if (speechInput.isConnected) { speechInput.stop() } else { speechInput.start() }
          onClick?.(e)
        }}
    ...
    return (
      <div
        ...
        className={cn(
          ...
          speechInput.isConnected ? "w-40 opacity-100" : "w-0 opacity-0",
          className
        )}
        title={displayText}
